...
<section class="py-8 mt-6 lg:mt-2 lg:py-16 antialiased">
    <div class="max-w-2xl mx-auto px-4 my-8">
        ...
        if(isset($_POST['post_id'])) {
            ...
            $getQuoteQuery = "SELECT * FROM posts WHERE post_id = $post_id AND user_id = $userid";
            ...
            if ($quoteResult && mysqli_num_rows($quoteResult) > 0) {
                ...
                echo '<article class="p-6 cursor-default md:mx-8 mt-24 lg:mt-5 border shadow-2xl text-base bg-white rounded-lg">
                    <footer class="flex justify-between items-center mb-2">
                        ...
                        <a onclick="document.getElementById(`deleteForm`).submit();" class="button text-center text-zinc-600 w-1/2 bg-white border-2 py-2 px-6 focus:outline-zinc-600 hover:bg-zinc-700 hover:text-white rounded-lg text-lg transition ease-in-out delay-200 hover:scale-110 hover:-translate-y-1 duration-300">Delete</a>

<form id="deleteForm" method="post" action="./confirm.php">
    ...
    <input type="hidden" name="post_id" value="'.$post_id.'>">
                ...
                echo "<script>alert('Post not found or post does not belong to current user');</script>";
