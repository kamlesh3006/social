...
if(isset($_POST["email"])){
    ...
    if($result){
        ...
        if($check == $_POST["food"]){
            ...
            echo "<script>alert('You can now update your password.')</script>";
            echo"<script>window.location.href='./update_password.php'</script>";
            exit();
        }else{
            echo "<script>alert('Answer didn\'t match!')</script>";
        }
    }else{
...
<head>
    ...
    <title>MuseWords - Change Password</title>
    <style>
        button {
...
<body style="background-color: #FAFBFB; display: flex; flex-direction: column;">
<div class="content">
...
<div class=" mt-56 bg-white flex flex-col border rounded-lg w-full md:w-1/2 lg:w-1/3 justify-center items-center m-auto">
    ...
    <form class="w-2/3 border-t flex flex-col justify-center items-center" method="post" action="">
        <label class="mt-4 mb-2 text-gray-600 font-semibold" for="email">Enter your email id : </label>
        <input class="border w-full rounded py-1 px-2 mt-2 focus:outline " placeholder="example@email.com" type="email" id="email" name="email" required>
        <label class="mt-4 mb-2 text-gray-600 font-semibold" for="food">What is your favourite food?</label>
        <input class="border w-full rounded py-1 px-2 mt-2 focus:outline " placeholder="Answer is case sensitive." type="text" id="food" name="food" required>
